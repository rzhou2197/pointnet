...
parser.add_argument('--max_epoch', type=int, default=20, help='Epoch to run [default: 251]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            label_r = tf.reshape(labels_pl, [-1, 95])
            logits = tf.reshape(tf.argmax(logits_flat,1), [-1, 95])
            correct = tf.equal(logits, tf.to_int64(label_r))
            accuracy = tf.reduce_sum(tf.cast(correct, tf.float32)) / (float(BATCH_SIZE)*95)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    while TRAIN_DATASET.has_next_batch():
        ...
        pred_r = np.reshape(pred_val.rnn_output, [-1, 95, 51])
        pred_val = np.argmax(pred_r, 2)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    log_string('eval avg_accuracy: %f' % (total_correct / float(total_seen)))
    log_string('eval real_accuracy: %f' % (correct / (95 * bsize)))
    EPOCH_CNT += 1
